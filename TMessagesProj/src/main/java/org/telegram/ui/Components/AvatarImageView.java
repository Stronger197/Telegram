...
public class AvatarImageView extends BackupImageView {
    ...
    public static final int STORIES_INSETS = (int) AndroidUtilities.dpf2(3.5f);
    ...
    public int getInsets() {
        return hasStories ? STORIES_INSETS : 0;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float inset = hasStories ? STORIES_INSETS : 0;
        ...
        ImageReceiver blurReceiver = getBlurImageReceiver();

        
        if (blurReceiver != null && blurReceiver.getAlpha() > 0) {

            
            
            if (blurReceiver.getBitmap() == null) {
                
                if (imageReceiver.getBitmap() != null && !imageReceiver.getBitmap().isRecycled()) {
                    
                    blurReceiver.setImageBitmap(Utilities.stackBlurBitmapMax(imageReceiver.getBitmap()));
                }
            }
            ...
            if (blurReceiver.getBitmap() != null) {
                ...
                blurReceiver.setImageCoords(imageReceiver.getImageX(), imageReceiver.getImageY(), imageReceiver.getImageWidth(), imageReceiver.getImageHeight());
                blurReceiver.setRoundRadius(imageReceiver.getRoundRadius());
                blurReceiver.draw(canvas);
            }
        }
